             ...
             <form id="filter-form">
                 ...
                 <div class="row">
                     ...
                     <div class="col-sm-3">
                         <div class="form-group">
                             <label> Service</label>
                             <Select2 v-model="query.service_id" placeholder="Select Service" :options="data.services"  />
                         </div>
                     </div>
                     <div class="col-md-2">
                         <div class="form-group">
                                ...
                                <td class="text-right">
                                    <div class="dropdown dropdown-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                             ...
                                             <jet-responsive-nav-link :href="route('opdBillings.edit', [row.id])"  class="dropdown-item"   v-if="row.status != 'COMPLETE' && ($page.auth.user.role == 'BM' || $page.isAdmin )">
                                                <i class="fa fa-pencil m-r-5"></i>Edit
                                            </jet-responsive-nav-link>

                                            <jet-responsive-nav-link :href="route('opd_add_refund', [row.id])"  class="dropdown-item" v-if="row.status == 'COMPLETE' && row.refund_status == 0 && ($page.auth.user.role == 'BM' || $page.auth.user.role == 'BE' || $page.isAdmin )">
...
import Select2 from 'v-select2-component';
 ...
 export default {
     components: {
            JetResponsiveNavLink, Invoice, Select2,
         datetime: Datetime
        },
        props: ['data', 'errors'],
        data() {
     ...
     mounted() {
        ...
        methods: {
            ...
            resetData: function () {
                this.resetQuery()
                this.$inertia.get('/opdBillings', this.query)
            },
            getData: function () {
                this.$inertia.get('/opdBillings', this.query)
            },
            ...
            invoice: function (bill) {
                HTTP.get('/opdBillings/'+bill.id+'/receipt')
                    .then((response)=>{
